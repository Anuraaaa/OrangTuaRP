...
CMD:faction(playerid, params[])
{
	...
	if (sscanf(params, "s[24]S()[128]", type, string))
		return SendSyntaxMessage(playerid, "/faction [Names]"), SendClientMessage(playerid, COLOR_SERVER, "Names: {FFFFFF}locker, invite, kick, menu, accept, setrank, offkick, resetduty");
	...
	if(!strcmp(type, "locker", true))
	{
	...
	else if(!strcmp(type, "offkick", true)) {
		...
		if(isnull(string))
			return SendSyntaxMessage(playerid, "/faction [offkick] [character name]");

		new is_online = INVALID_PLAYER_ID;
		foreach(new i : Player) if(!strcmp(GetName(i), string)) {
			is_online = i;
			break;
		}

		if(is_online != INVALID_PLAYER_ID) {
			return SendErrorMessage(playerid, "Player tersebut sedang online!");
		}

		new Cache:result = mysql_query(sqlcon, sprintf("SELECT * FROM `characters` WHERE `Name` = '%s' LIMIT 1;", string));
		...
		if(cache_num_rows()) {
			...
			if(faction != PlayerData[playerid][pFaction]) {
				SendErrorMessage(playerid, "Player tersebut bukan pada faction-mu!");
			}
			else {

				SendServerMessage(playerid, "You have kicked %s from your faction!", string);

				new query[256];
				mysql_format(sqlcon, query, sizeof(query), "UPDATE `characters` SET `Faction` = '-1', `FactionID` = '-1', `FactionRank` = '0', `Onduty` = '0' WHERE `Name` = '%e'", string);
				mysql_tquery(sqlcon, query);
			}
		}
		else SendErrorMessage(playerid, "Tidak dapat menemukan karakter tersebut.");
	...
	else if(!strcmp(type, "menu", true))
	{
	...
	else if(!strcmp(type, "resetduty", true)) {
		...
		if(GetFactionType(playerid) != FACTION_POLICE && GetFactionType(playerid) != FACTION_MEDIC && GetFactionType(playerid) != FACTION_GOV && GetFactionType(playerid) != FACTION_NEWS)
			return SendErrorMessage(playerid, "This faction can't reset duty time.");
		...
		foreach(new i : Player) if(PlayerData[i][pFaction] == PlayerData[playerid][pFaction]) {
			PlayerData[i][pFactionHour] = 0;
			PlayerData[i][pFactionMinute] = 0;
			PlayerData[i][pFactionSecond] = 0;
		}

		new query[256];
		mysql_format(sqlcon, query, sizeof(query), "UPDATE `characters` SET `FactionHour` = '0', `FactionMinute` = '0', `FactionSecond` = '0' WHERE `FactionID` = '%d'", PlayerData[playerid][pFactionID]);
		mysql_tquery(sqlcon, query);

		SendServerMessage(playerid, "You have reset all duty time of your faction member.");

	}
	else if(!strcmp(type, "setbadge", true)) {
		...
		if(sscanf(string, "ud", targetid, badge))
			return SendSyntaxMessage(playerid, "/faction [playerid/PartOfName] [setbadge] [new badge]");
		...
		PlayerData[targetid][pFactionBadge] = badge;
		SendServerMessage(playerid, "You have set "YELLOW"%s's "WHITE"badge to %d.", ReturnName(targetid), badge);
		SendServerMessage(targetid, "Your badge has been adjusted to %d by "YELLOW"%s", badge, ReturnName(playerid));
	}
	...
	else if(!strcmp(type, "quit", true)) {
		
		if(GetFactionType(playerid) == FACTION_NONE)
			return SendErrorMessage(playerid, "You are not in any faction.");

		SendServerMessage(playerid, "You have quit from your faction (%s)", Faction_GetName(playerid));
