...
CMD:pr(playerid, params[])
{
 	...
 	foreach(new i : Player) if(PlayerData[i][pChannel] == PlayerData[playerid][pChannel] && Inventory_Count(i, "Portable Radio"))
	{
		if(strlen(params) > 64)
		{
			...
			format(string, sizeof(string), "...%s", PlayerData[playerid][pChannel], GetName(playerid, false), params[64]);
			SendClientMessageEx(i, 0xFFFF90FF, string);
		}
		else
		{
...
CMD:o(playerid, params[])
{
    ...
    foreach (new i : Player) if(!PlayerData[i][pTogGlobal] && IsPlayerSpawned(i))
    {
        ...
        else
        {
			...
			SendClientMessageEx(i, -1, "(( %s%s"WHITE" (%d): %s ))", chat_color[vip_level], GetName(playerid, false), playerid, params);
        }
    }
    return 1;
}
...
CMD:ado(playerid, params[])
{
	...
	if (sscanf(params,"s[128]",msg))
		return SendClientMessage(playerid,COLOR_GREY, "USAGE: {FFFFFF}/ado [text]");
...
CMD:v(playerid, params[])
{
	...
	else if(!strcmp(type, "light", true))
	{
		...
		new Cache:result = mysql_query(sqlcon, sprintf("SELECT * FROM `vehicle` WHERE `vehState` = '%d' AND `vehExtraID` = '%d'", VEHICLE_STATE_STUCK, PlayerData[playerid][pID]));
	...
	else if(!strcmp(type, "lock", true))
	{
		...
		else
		{
			...
			foreach(new i : Vehicle) if(Vehicle_GetType(i) == VEHICLE_TYPE_PLAYER && Vehicle_IsShared(playerid, i)) {
