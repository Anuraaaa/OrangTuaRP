...
ptask Player_CharacterCough[1000](playerid)
{
    ...
    if(Character_CanUpdate(playerid))
    {
        if(PlayerData[playerid][pCough] > 0)
        {
...
task EnergyUpdate[90000]()
{
	foreach(new playerid : Player) if(PlayerData[playerid][pSpawned] && !PlayerData[playerid][pAduty] && PlayerData[playerid][pJailTime] < 1)
	{
...
{
	foreach(new i : Vehicle) if(IsValidVehicle(i) && VehicleData[i][vType] == VEHICLE_TYPE_RENTAL && VehicleData[i][vRental] != -1)
	{
			...
			if(PlayerData[playerid][pSecond] >= 60)
			{
				...
				if(PlayerData[playerid][pMinute] >= 60)
				{
					...
					if(PlayerData[playerid][pJobduty]) {
						...
						SendServerMessage(playerid, "Kamu mendapatkan "GREEN"$50.00 "WHITE"pada /salary dari Job duty.");
						AddSalary(playerid, "Job Duty", 5000);
					}
...
ptask DelayUpdate[1000](playerid)
{
	if(PlayerData[playerid][pSpawned])
	{
		if(GetFactionType(playerid) != FACTION_NONE && PlayerData[playerid][pOnDuty])
		{
