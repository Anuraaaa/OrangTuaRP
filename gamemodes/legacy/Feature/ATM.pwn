...
stock ATM_Refresh(atmid)
{
	if (atmid != -1 && AtmData[atmid][atmExists])
	{
		...
		Streamer_SetIntData(STREAMER_TYPE_AREA, AtmData[atmid][atmArea], E_STREAMER_EXTRA_ID, AREA_TYPE_ATM);
...
hook OnPlayerEnterDynArea(playerid, STREAMER_TAG_AREA:areaid) {
	for(new i = 0; i < MAX_ATM; i++) if(AtmData[i][atmExists])
	{
		if(areaid == AtmData[i][atmArea] && Streamer_GetIntData(STREAMER_TYPE_AREA, areaid, E_STREAMER_EXTRA_ID) == AREA_TYPE_ATM)
		{
