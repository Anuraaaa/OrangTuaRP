...
ShowPlayerInfo(playerid, targetid) {
	...
	new string[178];
	format(string, sizeof(string), "ID: %i (UCP:%s)", targetid, GetUsername(targetid));
...
UpdatePlayerInfo(playerid, targetid) {
	...
	if (GetWeapon(targetid) == 0) {
	    string = "WEAPON: ~w~Unarmed";
	}
	else {
		format(string, sizeof(string), "WEAPON: ~w~%s (%i)", ReturnWeaponName(PlayerData[targetid][pGuns][g_aWeaponSlots[GetWeapon(playerid)]]), PlayerData[targetid][pAmmo][g_aWeaponSlots[GetWeapon(playerid)]]);
	}
	PlayerTextDrawSetString(playerid, playerInfoPTD[playerid][4], string);

	format(string, sizeof(string), "MONEY: ~g~$~w~%s", FormatNumber(GetMoney(targetid)));
...
ShowVehicleInfo(playerid, vehicleid) {
	...
	PlayerTextDrawSetString(playerid, vehicleInfoPTD[playerid][0], sprintf("%s", RemoveSpace(GetVehicleName(vehicleid))));
...
hook OnPlayerClickTD(playerid, Text:clickedid) {
	if (spectateID[playerid] != INVALID_PLAYER_ID) {
		if (clickedid == Text:INVALID_TEXT_DRAW) {
        	...
        	GameTextForPlayer(playerid, "~w~Spectate Camera Mode: ~g~ON~n~~w~You can rotate camera with your mouse~n~~w~Type ~y~/mousemode ~w~to switch to mouse", 5000, 3);
		}
...
ptask timer_OnPlayerSpectating[1000](playerid) {
...
CMD:spec(playerid, params[]) {
	...
	if (GetPlayerState(targetid) == PLAYER_STATE_WASTED || GetPlayerState(targetid) == PLAYER_STATE_SPECTATING) {
		return SendErrorMessage(playerid, "Player isn't spawned at the moment.");
	}
    ...
    SendClientMessage(playerid, 0xAAAAAAFF, "Gunakan /mousemode untuk mengaktifkan/menonaktifkan cursor untuk rotasi layar.");
	return 1;
}

CMD:unspec(playerid, params[]) {
	if (GetPlayerState(playerid) != PLAYER_STATE_SPECTATING) {
		return SendErrorMessage(playerid, "You appear to be not spectating at the moment.");
	}
