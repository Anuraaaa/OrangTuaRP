...
stock Gate_Operate(gateid, playerid = INVALID_PLAYER_ID)
{
	if (gateid != -1 && GateData[gateid][gateExists])
	{
		...
		if (!GateData[gateid][gateOpened])
		{
			...
			if(playerid != INVALID_PLAYER_ID)
				ShowMessage(playerid, sprintf("[ID:%d] Gate ~g~opened", gateid), 2);
		}
		else if (GateData[gateid][gateOpened])
		{
			...
			if(playerid != INVALID_PLAYER_ID) 
				ShowMessage(playerid, sprintf("[ID:%d] Gate ~r~closed", gateid), 2);
		}
	}
	return 1;
}
...
CMD:gate(playerid, params[])
{
	...
	if (id != -1)
	{
		...
		else
		{
			...
			{
			    case 0:
				    ShowMessage(playerid, sprintf("[ID:%d] Gate ~r~closed", id), 2);

                case 1:
				    ShowMessage(playerid, sprintf("[ID:%d] Gate ~g~opened", id), 2);
			}
		}
	}
	return 1;
