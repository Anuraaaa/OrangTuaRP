...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	if(dialogid == DIALOG_DEALER_BUY)
	{
		if(response)
		{
			...
			new vehicleid = Vehicle_Create(DealerData[id][dealerVehicle][listitem], DealerData[id][dealerSpawn][0], DealerData[id][dealerSpawn][1], DealerData[id][dealerSpawn][2], DealerData[id][dealerSpawn][3], random(126), random(126));
			Vehicle_SetOwner(vehicleid, playerid, true);
			Vehicle_SetType(vehicleid, VEHICLE_TYPE_PLAYER);
			VehicleData[vehicleid][vPrice] = DealerData[id][dealerCost][listitem];
			...
			GiveMoney(playerid, -DealerData[id][dealerCost][listitem], "Membeli kendaraan");
...
/*	if(dialogid == DIALOG_ORIGIN)
	...
	if(dialogid == DIALOG_FAMILY_MENU)
	{
		if(response)
	    {
			...
			switch(listitem)
			{
				case 0:
				{
					Family_OpenStorage(playerid, familyid);	
				}
				case 1:
				{
	...
	if(dialogid == DIALOG_FAMILY_STORAGE)
	{
		if(response)
	    {
			static
		    ...
		    if ((familyid = PlayerData[playerid][pFamily]) != -1)
			{
				if (listitem == 0)
				{
				...
				{
					Family_WeaponStorage(playerid, familyid);
				}
			}	
		}
	}
	if(dialogid == DIALOG_FAMILY_WEAPON)
	{
		if(response)
	    {
			static
		    ...
		    if ((familyid = PlayerData[playerid][pFamily]) != -1)
			{
			    if (FamilyData[familyid][familyWeapons][listitem] != 0)
			    {
			        if(PlayerData[playerid][pLevel] < 3)
			            return SendErrorMessage(playerid, "You must level 3 first to holding weapon.");
			            
			        if(PlayerHasWeapon(playerid, FamilyData[familyid][familyWeapons][listitem]))
			            return SendErrorMessage(playerid, "You already have this type of weapon!");
	          ...
	          //      Log_Write("logs/storage_log.txt", "[%s] %s has taken a \"%s\" from house ID: %d (owner: %s).", ReturnDate(), ReturnName(playerid), ReturnWeaponName(HouseData[houseid][houseWeapons][listitem]), HouseData[houseid][houseID], (House_IsOwner(playerid, houseid)) ? ("Yes") : ("No"));
					...
					FamilyData[familyid][familyAmmo][listitem] = 0;
					FamilyData[familyid][familyDurability][listitem] = 0;
				...
				else
				{
					...
					FamilyData[familyid][familyDurability][listitem] = PlayerData[playerid][pDurability][g_aWeaponSlots[weaponid]];
	...
	if(dialogid == DIALOG_FAMILY_ITEM)
	{
		new familyid = PlayerData[playerid][pFamily], string[156];
		if (response)
		{
    		if (FamilyStorage[familyid][listitem][fItemExists])
			{
   				PlayerData[playerid][pListitem] = listitem;
   				PlayerData[playerid][pListitem] = listitem;

				strunpack(string, FamilyStorage[familyid][PlayerData[playerid][pListitem]][fItemName]);

				format(string, sizeof(string), "%s (Amount: %d)", string, FamilyStorage[familyid][listitem][fItemQuantity]);
				ShowPlayerDialog(playerid, DIALOG_FAMILY_OPTION, DIALOG_STYLE_LIST, string, "Take Item\nStore Item", "Select", "Back");
			}
			else
			{
			    PlayerData[playerid][pStorageSelect] = 2;
	...
	if(dialogid == DIALOG_FAMILY_OPTION)
	{
		new
	    ...
	    if(response)
	    {
		    ...
		    strunpack(string, FamilyStorage[familyid][itemid][fItemName]);
			if (response)
			{
				switch (listitem)
				{
				    case 0:
				    {
				        if (FamilyStorage[familyid][itemid][fItemQuantity] == 1)
				        {
				            new id = Inventory_Add(playerid, string, FamilyStorage[familyid][itemid][fItemModel], 1);
				        ...
				        {
				            new str[128];
				            format(str, sizeof(str), "Item: %s (Amount: %d)\n\nPlease enter the amount that you wish to take for this item:", string, FamilyStorage[familyid][itemid][fItemQuantity]);
				            ShowPlayerDialog(playerid, DIALOG_FAMILY_WITHDRAW, DIALOG_STYLE_INPUT, "Family Take", str, "Take", "Back");
				        }
				    }
					case 1:
					{
						...
						else if (InventoryData[playerid][id][invQuantity] > 1)
						{
							...
							new str[128];
							format(str, sizeof(str), "Item: %s (Amount: %d)\n\nPlease enter the quantity that you wish to store for this item:" , string, InventoryData[playerid][PlayerData[playerid][pListitem]][invQuantity]);
	                        ShowPlayerDialog(playerid, DIALOG_FAMILY_DEPOSIT, DIALOG_STYLE_INPUT, "Family Deposit", str, "Store", "Back");
						}
					}
				}
			}
	...
	if(dialogid == DIALOG_FAMILY_WITHDRAW)
	{
		new
		    familyid = PlayerData[playerid][pFamily],
		    string[32];
	    ...
	    if(response)
	    {
			...
			if (amount < 1 || amount > FamilyStorage[familyid][PlayerData[playerid][pListitem]][fItemQuantity])
			{
			    new str[152];
			    format(str, sizeof(str), "Item: %s (Amount: %d)\n\nPlease enter the quantity that you wish to take for this item:", string, FamilyStorage[familyid][PlayerData[playerid][pListitem]][fItemQuantity]);
				ShowPlayerDialog(playerid, DIALOG_FAMILY_WITHDRAW, DIALOG_STYLE_INPUT, "Family Take", str, "Take", "Back");
				return 1;
			}
			new id = Inventory_Add(playerid, string, FamilyStorage[familyid][PlayerData[playerid][pListitem]][fItemModel], amount);
	...
	if(dialogid == DIALOG_FAMILY_DEPOSIT)
	{
		new
	        familyid = PlayerData[playerid][pFamily],
	        string[32];
...
public OnPlayerSpawn(playerid)
{
	...
	{	
		...
		foreach(new i : Player) if(!PlayerData[i][pTogLogin])
		{
			...
			format(str, sizeof(str), "* "YELLOW"%s "WHITE"is now connected to "SERVER_NAME"%s.", ReturnName(playerid), !strcmp(country, "Unknown", true) ? ("") : sprintf("(%s, %s)", city, country));
...
stock ReturnWeaponCount(playerid)
{
	new
		count,
	    weapon,
...
stock CountPlayerShootWound(playerid) {
...
CountPlayerVehicleSlot(playerid) {
	...
	if(IsPlayerHouseGuest(playerid))
		slot += 1;
...
timer EnterWorkshop[5000](playerid, wsidx, vehicleid) {
	...
	if(!Iter_Contains(Workshop, wsidx))
		return 0;
	...
	SetVehicleInterior(vehicleid, 2);
	SetVehicleVirtualWorld(vehicleid, WorkshopData[wsidx][wsWorld]);
	SetVehiclePos(vehicleid, 1424.1827,1311.7906,10.5928);
	SetVehicleZAngle(vehicleid, 0.2209);

	foreach(new i : VehicleOccupant(vehicleid)){
		SetPlayerVirtualWorld(i, WorkshopData[wsidx][wsWorld]);
		SetPlayerInterior(i, 2);
		PlayerData[i][pInWorkshop] = wsidx;
		Streamer_Update(playerid, STREAMER_TYPE_OBJECT);
	}
	return 1;
}
